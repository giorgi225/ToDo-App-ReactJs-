    ...
    return (
        ...
        ?
            props.todos.map(item => (
                <div key={item.id} className="flex flex-col border-t-[1px] border-white-600">
                <div className="select-none w-full border-b-[1px] border-white-600 px-14 group py-3 hover:bg-white-500 transition-all">
                    <div className="w-full flex items-center justify-between">
                    <div onClick={()=> props.toggleIsDone(item.id)} className="flex items-center gap-3 cursor-pointer">
                        {item.isDone
                        ?
                        <div className="flex items-center justify-center min-w-[16px] min-h-[16px] rounded-full border-2 border-main bg-main">
                            <div className="animation border-transparent border-b-2 border-r-2 border-main rotate-45 border-white"></div>
                        </div>
                        :
                        <div className="min-w-[16px] min-h-[16px] rounded-full border-2 border-main"></div>
                        }
                        <div className={item.isDone ? "line-through flex flex-col items-start" : "flex flex-col items-start"}>
                            <div className="flex items-center gap-2">
                                <h4 className="text-base font-bold text-main">{item.title}</h4>
                                <p className="text-sm font-light text-black">
                                    {
                                    item.createdAt +" at "+ item.createdTime
                                    }
                                </p>
                            </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 transition-all group-hover:opacity-100">
                        <div onClick={()=> props.editTask(item.id)} className="p-2 group/edit cursor-pointer hover:scale-[1.1] transition-all">
                        ...
                        <div onClick={()=> props.deleteTask(item.id)} className="p-2 group/edit cursor-pointer hover:scale-[1.1] transition-all">
            ...
            <div className="flex items-center justify-center py-12">
                <h5 className="text-black text-lg">No Tasks To Show</h5>
            </div>
    }
        </div>
    )
